...
sftp = sftpop.Sftp(hostname=host, username='username', password='password', port=5522, cnopts=cnopts)
sftp.connect()
download_loc = 'D:/Files/Input/Journey_break_point'
source_loc = './'
source_loc_H = 'NFHR/'
recipt_loc = '../KPBI/'
# source_loc = './'
    ...
    for filename in list(target_file):
        try:
            if historical == 'Receipt':
                ...
                if "New_List_Of_Unique_Users" in filename:
...
def main(datafile, table, query, cursor, historical=False):
    ...
    if historical == False or historical == 'Receipt':
...
os.system(f'''cmd /c sqlcmd -Q " {SP} " -o "D:\SAP_files\Logs\Validation\RJ.csv" -s "," ''')
